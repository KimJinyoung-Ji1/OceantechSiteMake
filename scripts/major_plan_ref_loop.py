...
def load_master_targets() -> list[dict[str, str]]:
    ...
    with MASTER_TARGETS.open("r", encoding="utf-8") as handle:
        ...
        for row in reader:
            if len(row) < 2:
                continue
            rows.append({"name": row[0].replace("\ufeff", "").strip(), "url": row[1].strip()})
    return rows
...
def cleanup_review_against_ref() -> int:
    ...
    if not REVIEW_DIR.exists():
        return 0
    removed = 0
    for path in list(REVIEW_DIR.iterdir()):
...
def build_active_targets() -> list[dict[str, str]]:
    ...
    rows = [row for row in load_master_targets() if row["name"] not in ref and row["name"] not in verified]
...
def write_status(cycle: int, active_rows: list[dict[str, str]], collect_logs: list[str], validator_log: str, phase: str, removed_review: int) -> None:
    ...
    lines.append("")
    lines.append("[루프 대상]")
    for row in active_rows[:80]:
        lines.append(f"- {row['name']}\t{row['url']}")
    lines.append("")
    lines.append("[수집 로그]")
    lines.extend(collect_logs[:50])
    lines.append("")
    lines.append("[검증 로그]")
